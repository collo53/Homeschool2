...
export default function TeacherAssignmentHero() {
    ...
    const fetchAssignments = async () => {
      try {
        ...
        const response = await axios.get(
          `http://127.0.0.1:8000/Hub/assignments/teacher/${teacherId}/`
        );
        setAssignments(
          response.data.map((a) => ({
            ...a,
            class: a.class_name,
            dueDate: a.due_date,
            fileURL: a.file ? `http://127.0.0.1:8000${a.file}` : null,
          }))
        );
      } catch (error) {
    ...
    try {
      if (isEditing && selectedAssignment) {
        const response = await axios.put(
          `http://127.0.0.1:8000/Hub/assignments/${selectedAssignment.id}/`,
          formData,
        ...
        const response = await axios.post(
          "http://127.0.0.1:8000/Hub/assignments/",
          formData,
          { headers: { "Content-Type": "multipart/form-data" } }
        );
        ...
        setAssignments((prev) => [
          ...prev,
          {
            ...
            fileURL: addedAssignment.file
              ? `http://127.0.0.1:8000${addedAssignment.file}`
              : null,
          },
        ]);
      }
  ...
  const viewSubmissions = async (assignment) => {
    try {
      const response = await axios.get(
        `http://127.0.0.1:8000/Hub/assignments/${assignment.id}/submissions/`
      );
