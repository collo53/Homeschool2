...
function TeacherProfileModal({ teacher, onClose, setTeachers }) {
  ...
  useEffect(() => {
    const fetchStudents = async () => {
      try {
        const response = await axios.get("http://localhost:8000/Hub/getstudents/");
  ...
  const handleSave = async () => {
    try {
      await axios.post("http://localhost:8000/Hub/assign-students/", {
  ...
  try {
    await axios.delete(`http://localhost:8000/Hub/delete-teacher/${teacher.id}/`);
